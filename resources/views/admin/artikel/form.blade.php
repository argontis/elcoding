...
<div class="surface-card p-6 max-w-3xl">
    ...
    <form action="{{ isset($data) ? url('admin/artikel/'.$data->id) : url('admin/artikel') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Konten Artikel</label>
                <textarea name="content" rows="10" class="rich-text w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50" required placeholder="Tulis konten artikel di sini...">{{ $data->content ?? '' }}</textarea>
            </div>
        </div>
        <div class="mt-8 flex gap-3 border-t border-slate-100 pt-5">
            ...
            <button type="submit" class="btn-primary px-5 py-2.5 rounded-xl font-semibold flex items-center justify-center gap-2 shadow-md">Simpan Artikel</button>
        </div>
    </form>
</div>
@endsection
