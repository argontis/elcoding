...
<div class="surface-card">
    <div class="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a href="{{ url('admin/artikel/create') }}" class="btn-primary px-5 py-2.5 rounded-xl font-semibold flex items-center justify-center gap-2">
            <i class="fas fa-plus"></i> Tulis Blog Baru
        </a>
    </div>
    ...
    <form method="GET" action="{{ url('admin/artikel') }}" class="p-4 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50 border-b border-slate-100">
        <div class="relative w-full sm:w-72">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul blog..." class="w-full sm:w-80 px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50" onchange="this.form.submit()">
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse modern-table">
            ...
            <tbody>
                ...
                @empty
                <tr>
                    <td colspan="7" class="py-12 text-center text-slate-500">Belum ada blog yang diterbitkan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
