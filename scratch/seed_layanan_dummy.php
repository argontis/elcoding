...
echo "Updating/creating dummy Layanan records...\n";
foreach ($dummyLayanans as $data) {
    $layanan = Layanan::updateOrCreate(
        ['slug' => $data['slug']],
        $data
    );
    echo "ID: {$layanan->id} | Title: {$layanan->title} | Price: {$layanan->price}\n";
}

echo "\nTotal Layanan in database: " . Layanan::count() . "\n";
